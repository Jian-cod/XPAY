...
export async function GET(request: NextRequest) {
  try {
    ...
    const cpagripUrl = `https://www.cpagrip.com/common/offer_feed_rss.php?user_id=2526699&key=312f554801f00416c51fbf168eed2e6c&limit=30&tracking_id=${encodeURIComponent(userId)}`;
    ...
    return NextResponse.json({ offers, source: offers.length > 0 ? 'cpagrip' : 'empty' });
    
  } catch (error: any) {
    console.error('Offers fetch error:', error);
    return NextResponse.json({ offers: [], source: 'error', error: error.message });
  }
}
